...
export const LandingPage: React.FC<LandingPageProps> = ({ onStart }) => {
  ...
  const fullText = "Prepare yourself to conquer your AWS certification";
  ...
  return (
    <div className="min-h-screen bg-[#0a0a12] text-slate-100 relative overflow-hidden">
      ...
      <main className="relative z-10">
        <section className="pt-8 pb-16 px-4">
          <div className="max-w-6xl mx-auto text-center">
            <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
              {/* Glitch title */}
              <h1 className="glitch mb-8" data-text="MASTER THE CLOUD">
                MASTER THE CLOUD
              </h1>
              ...
              <div className="mb-8">
                <span
                  className="blink-tag inline-block px-8 py-3 text-xl uppercase tracking-widest font-bold"
                  style={{
              ...
              {/* Typewriter text */}
              <p className="vt323-text text-slate-200 mb-2 min-h-[2.5rem]">
                {displayedText}
                <span className="typewriter-cursor" />
              </p>

              <p className="vt323-text text-slate-400 mb-12 text-lg">
                Cloud Practitioner · Solutions Architect · AI Practitioner
              </p>

              {/* Pixel button */}
              <motion.div whileTap={{ scale: 0.95 }}>
                <button onClick={handleScrollToCerts} className="pixel-btn">
                  START NOW
                </button>
              </motion.div>
            </motion.div>
        ...
        <section id="certification-section" className="py-12 px-4 bg-[#0a0a12]/80">
          <div className="max-w-6xl mx-auto">
            ...
            >
              {isLoading && (
                <div className="border-2 border-[#00FFFF] bg-[#0a0a12]/90 px-6 py-10 text-center vt323-text text-slate-300 text-xl">
                ...
                certifications.map((cert) => {
                  ...
                  return (
                    <button
                      ...
                      className="group relative w-full overflow-hidden border-2 bg-[#0a0a12]/70 px-8 py-6 text-left transition-all duration-200 hover:bg-[#0a0a12]/90"
                      style={{
                        borderColor: `${colors.border}80`,
                      }}
                      onMouseEnter={(e) => {
                        e.currentTarget.style.borderColor = colors.borderHover;
                        e.currentTarget.style.boxShadow = `0 0 30px ${colors.shadow}`;
                      }}
                      onMouseLeave={(e) => {
                      ...
                      <div className="flex flex-col gap-4">
                        <div className="flex flex-wrap items-center justify-between gap-4">
                          <div className="space-y-3">
                            {examCode && (
                              <div
                                className="inline-block px-3 py-1 text-xs font-bold uppercase tracking-[0.3em] border-2"
                                style={{
                                  fontFamily: 'VT323, monospace',
                                  color: colors.badge,
                            ...
                            <div
                              className="vt323-text text-2xl md:text-3xl font-bold text-white"
                            >
                              {cert.name}
                            </div>
                          </div>
                          <div
                            className="vt323-text text-lg font-semibold transition-all duration-200"
                            style={{ color: colors.text }}
                          >
                            INICIAR &rarr;
                          </div>
                        </div>
                        <p className="vt323-text text-base leading-relaxed text-slate-300 group-hover:text-white">
                          {cert.description}
                        </p>
                      </div>
                    </button>
              ...
              {!isLoading && certifications.length === 0 && (
                <div className="border-2 border-[#00FFFF] bg-[#0a0a12]/90 px-6 py-10 text-center vt323-text text-slate-300 text-xl">
                  Nenhuma certificação encontrada. Verifique sua conexão.
                </div>
              )}
            </motion.div>
          </div>
              ...
              <div className="flex flex-col md:flex-row items-center gap-8">
                ...
                <div className="flex-1 text-center md:text-left">
                  ...
                  <p className="vt323-text text-lg uppercase tracking-widest text-[#00FFFF] mb-6">
                    2x AWS Certified · Platform Creator
                  </p>
                  <p className="vt323-text text-slate-200 mb-6 leading-relaxed text-lg">
                    I create free practice exams for AWS certifications. I developed this platform because I believe quality preparation is everyone's right.
                  </p>
                  <div className="flex flex-wrap items-center justify-center md:justify-start gap-4">
                    ...
                    <motion.a
                      ...
                      rel="noopener noreferrer"
                      whileTap={{ scale: 0.95 }}
                      className="inline-block px-6 py-3 border-2 border-[#FF9900] bg-[#FF9900] text-[#0a0a12] vt323-text text-lg font-bold uppercase transition-all duration-200 hover:bg-transparent hover:text-[#FF9900]"
                    >
                      KO-FI
                    </motion.a>
                  </div>
